...
-(void)reloadCollapseClickExceptIndex:(int)idx
{
	if (idx < 0 || idx >= self.dataArray.count) {
		[self reloadCollapseClick];
		return;
	}

    float totalHeight = 0;
	__strong id idxCellSave = self.dataArray[idx];
	CGPoint offsetSave = self.contentOffset;
    ...
    // Add cells
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        CollapseClickCell *cell;
		if (xx == idx) {
			cell = idxCellSave;
		} else {
			cell = [CollapseClickCell newCollapseClickCellWithTitle:[CollapseClickDelegate titleForCollapseClickAtIndex:xx]
															  index:xx
															content:[CollapseClickDelegate viewForCollapseClickContentViewAtIndex:xx]];
		...
		totalHeight += kCCHeaderHeight + kCCPad;
		if ([self.isClickedArray[xx] boolValue]) {
			totalHeight += cell.ContentView.frame.size.height + kCCPad;
		}
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
		if ([self.isClickedArray[xx] boolValue] && xx != idx) {
			[self closeCollapseClickCellAtIndex:xx animated:NO];
			[self openCollapseClickCellAtIndex:xx animated:NO];
		}
	}

    [self setContentSize:CGSizeMake(self.frame.size.width, totalHeight)];
    [self setContentOffset:offsetSave];
}

-(void)refreshCollapseClickTitle {
	for (int i = 0; i < self.dataArray.count; i++) {
		CollapseClickCell *cell = self.dataArray[i];
		cell.TitleLabel.text = [CollapseClickDelegate titleForCollapseClickAtIndex:i];
	}
}
...
-(void)openCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
    ...
    if ([[self.isClickedArray objectAtIndex:index] boolValue] != YES) {
		...
		if ([cell.content respondsToSelector:@selector(flashScrollIndicators)]) {
			[cell.content performSelector:@selector(flashScrollIndicators)];
		}
    }
}

-(void)openForceCollapseClickCellAtIndex:(int)index animated:(BOOL)animated {
	float duration = 0;
	if (animated) {
		duration = 0.25;
	}
